...
#define MYZWaterFallViewDefaultCellHeight 70
#define MYZWaterFallViewDefaultMargin 10
#define MYZWaterFallViewDefaultNumberOfColumns 3

#import "MYZWaterFallView.h"
#import "MYZWaterFallCell.h"

@interface MYZWaterFallView ()

/** 用数组来保存每个cell的frame值, 有顺序,用数组来保存 */
...
@implementation MYZWaterFallView
...
-(void)reloadData
{
    ...
    //清除字典, 显示在屏幕上的cell
    [self.displaysCells.allValues makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self.displaysCells removeAllObjects];
    //清除,保存cell的frame的数组
    ...
    //间距
    CGFloat topM = [self marginForType:MYZWaterFallViewMarginTypeTop];
    CGFloat bottomM = [self marginForType:MYZWaterFallViewMarginTypeBottom];
    CGFloat leftM = [self marginForType:MYZWaterFallViewMarginTypeLeft];
    
    CGFloat columnM = [self marginForType:MYZWaterFallViewMarginTypeColumn];
    CGFloat rowM = [self marginForType:MYZWaterFallViewMarginTypeRow];
    
    
    //宽度, 瀑布流的每个cell的宽度都是一样的
    CGFloat widthOfCell = [self widthOfCell];
    ...
    for (int i=0; i<numberOfCells; i++)
    {
        ...
        //显示cell
        //MYZWaterFallCell * cell = [self.dataSource waterFall:self cellAtIndex:i];
    ...
    contentH += bottomM;
    self.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, contentH);
...
- (void)layoutSubviews
{
    ...
    for (int i=0; i<numberOfCells; i++)
    {
        ...
        MYZWaterFallCell * cell = self.displaysCells[@(i)];
...
-(id)dequeueReusableCellWithIdentifier:(NSString *)identifier
{
    ...
    __block  MYZWaterFallCell * reusableCell = nil ;
    
    
    //在集合中找到带有相同标识的cell
    [self.reusableCells enumerateObjectsUsingBlock:^(MYZWaterFallCell * cell, BOOL *stop) {
    ...
    //__block  MYZWaterFallCell * reusableCell 指针指向cell, 不会被销毁
    if(reusableCell)
    {
...
-(CGFloat)widthOfCell
{
    ...
    //间距
    CGFloat leftM = [self marginForType:MYZWaterFallViewMarginTypeLeft];
    CGFloat rightM = [self marginForType:MYZWaterFallViewMarginTypeRight];
    CGFloat columnM = [self marginForType:MYZWaterFallViewMarginTypeColumn];
...
- (NSInteger)numberOfColumns
{
    ...
    else { return MYZWaterFallViewDefaultNumberOfColumns; }
}
...
- (CGFloat)heightAtIndex:(NSInteger)index
{
    ...
    else { return MYZWaterFallViewDefaultCellHeight; }
}


//询问代理cell之间的间距, 若没有返回默认值10
- (CGFloat)marginForType:(MYZWaterFallViewMarginType)type
{
    ...
    else { return MYZWaterFallViewDefaultMargin; }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.displaysCells enumerateKeysAndObjectsUsingBlock:^(id key, MYZWaterFallCell * cell, BOOL *stop) {
